...
export default function OrderMap({
  ...
  const center = agentLocation || pickupLocation || dropLocation || [13.028028, 80.017032];
  ...
  return (
    <MapContainer center={center as [number, number]} zoom={15} scrollWheelZoom={true} style={{ height: '100%', width: '100%', borderRadius: '1.5rem' }}>
      <TileLayer
      ...
      {/* 🌾 Farmer Marker */}
      {pickupLocation && (
        <Marker position={pickupLocation} icon={farmerIcon} zIndexOffset={800}>
          <Popup className="custom-popup">
            <div className="font-bold text-sm">🌾 Farm Pickup Location</div>
            <div className="text-xs text-slate-600">Collect fresh produce from farmer here</div>
          </Popup>
        </Marker>
      )}
      ...
      {/* 🏠 Buyer Marker */}
      {dropLocation && (
        <Marker position={dropLocation} icon={buyerIcon} zIndexOffset={900}>
          <Popup className="custom-popup">
            <div className="font-bold text-sm">🏠 Buyer Delivery Location</div>
            <div className="text-xs text-blue-600 font-semibold">Deliver order to buyer here</div>
          </Popup>
        </Marker>
      )}
      ...
      {agentLocation && (
        <Marker position={agentLocation} icon={agentIcon} zIndexOffset={1000}>
          <Popup className="custom-popup">
